...
class Session : public Owned<::z_owned_session_t> {
   ...
   public:
    ...
    std::variant<SharedShmProvider, ShmProviderNotReadyState> obtain_shm_provider(bool blocking = false) const {
        SharedShmProvider provider(zenoh::detail::null_object);
        ::z_shm_provider_state state;

        ::z_obtain_shm_provider(interop::as_loaned_c_ptr(*this), blocking, &provider._0, &state);
        switch (state) {
